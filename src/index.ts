...
const generateTestLines = (sessionData: SessionData) : TestLines => {
  ...
  while (
    ...
    let currentInput: TimeStamped = inputIndex < sessionData.input.length ? 
      sessionData.input[inputIndex] : { ts: Number.MAX_SAFE_INTEGER };
    let currentCheckpoint: TimeStamped = checkpointIndex < sessionData.checkpoints.length ? 
      sessionData.input[checkpointIndex] : { ts: Number.MAX_SAFE_INTEGER };
    let currentUserInteraction: TimeStamped = userInteractionIndex < sessionData.userInteractions.length ? 
      sessionData.input[userInteractionIndex] : { ts: Number.MAX_SAFE_INTEGER };
    let currentForegroundActivity: TimeStamped = foregroundActivityIndex < sessionData.foregroundActivities.length ? 
      sessionData.input[foregroundActivityIndex] : { ts: Number.MAX_SAFE_INTEGER };
    ...
    if (currentLine.ts != Number.MAX_SAFE_INTEGER) {
      switch (currentLine) {
        ...
        case currentForegroundActivity:
          testLines.push({ foregroundActivity: currentInput as ForegroundActivity });
