...
function help() {
	console.log("usage: node cli.js --session-url=SESSION_URL [--zip] [--apk-url=APK_URL]");
	process.exit(0);
}
...
request.get(options["session-url"], function(err, res, body) {
	...
	if (options["zip"]) {
		...
		request.get(options["apk-url"], function(err, res, apkBuffer) {
			session_generator.saveGeneratedAppiumTest("url.json", sessionData, apkBuffer, tmpFilename).then((response) => {
